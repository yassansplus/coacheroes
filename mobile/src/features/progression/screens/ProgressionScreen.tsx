...
export function ProgressionScreen({ onHome, onToday, onProgram, onCoach, onProfile }: { onHome: () => void; onToday: () => void; onProgram: () => void; onCoach: () => void; onProfile: () => void }) {
  ...
  function back() { if (sheet) setSheet(null); else if (page === 'home') onHome(); else setPage(page === 'compare' ? 'photos' : 'home'); }
  ...
  function compare() {
    const available = data.photos.filter(item => item.images[angle]);
    if (available.length < 2) { showHistory('Comparer deux photos', ['Ajoute deux photos du même angle à des dates différentes.']); return; }
    setBefore(available[0].id); setAfter(available[available.length - 1].id); setPage('compare');
  }
  function save() {
    if (sheet === 'weight') {
      ...
      setData(current => {
        ...
        const photo: ProgressPhoto = { id: existing?.id ?? `photo-${Date.now()}`, date, images: { ...existing?.images, ...images }, weight: current.weights.find(item => item.date === date)?.value, waist: current.measures.find(item => item.date === date)?.waist };
  ...
  return <View style={s.screen}><ScreenBackdrop /><SafeAreaView style={{ flex: 1 }}><View style={[s.frame, { flex: 1 }]}>
    ...
    <ScrollView ref={scroll} contentContainerStyle={[s.content, page === 'home' && { paddingTop: 18 }]} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
      ...
      {page === 'photos' ? <PhotoGallery photos={data.photos} angle={angle} onAngle={setAngle} onAdd={() => openForm('photo')} onCompare={compare} onDelete={setDeleting} /> : null}
      {page === 'compare' ? <ComparePhotos photos={data.photos} angle={angle} onAngle={setAngle} beforeId={before} afterId={after} onChoose={side => { setError(''); setSheet(side); }} onClose={() => setPage('photos')} /> : null}
    ...
    <BottomSheet visible={sheet !== null} onClose={() => setSheet(null)} title={sheetTitle} style={{ maxHeight: '90%' }} footer={form ? <Button text="Enregistrer" onPress={save} hapticFeedback /> : undefined}>
      <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" contentContainerStyle={{ gap: 14, paddingBottom: 12 }}>
        ...
        {sheet === 'before' || sheet === 'after' ? data.photos.filter(item => item.images[angle]).map(item => <Row key={item.id} title={dateLabel(item.date)} subtitle={item.example ? 'Photo d’exemple' : undefined} onPress={() => {
          const other = data.photos.find(photo => photo.id === (sheet === 'before' ? after : before));
          if (other && other.id !== item.id && other.date !== item.date) { const pair = sorted([item, other]); setBefore(pair[0].id); setAfter(pair[1].id); setSheet(null); }
    ...
    <AppModal visible={!!deleting} onClose={() => setDeleting(undefined)} title="Retirer ces photos ?" actions={<View style={{ gap: 10 }}><Button text="Conserver" onPress={() => setDeleting(undefined)} /><Button text="Retirer" variant="outline" onPress={() => { setData(current => ({ ...current, photos: current.photos.filter(item => item.id !== deleting) })); setDeleting(undefined); }} /></View>}><Text style={s.body}>La prise de vue sera retirée de cet aperçu. Le fichier original sur ton appareil ne sera pas supprimé.</Text></AppModal>
  </View>;
}
