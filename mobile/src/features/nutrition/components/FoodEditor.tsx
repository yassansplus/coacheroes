...
export function FoodEditor({ initial, moment, onSave, onRemove, onReplace }: { initial: Ingredient; moment: string; onSave: (value: Ingredient) => void; onRemove: () => void; onReplace: () => void }) {
  ...
  return <>
    <Card style={s.card}><View style={s.row}><FoodIcon name={food.icon} background={colors.accentSurface} size={62} /><View style={{ flex: 1 }}><Text style={s.heading}>{food.name}</Text><Text style={s.muted}>Dans {moment}</Text></View></View>{food.provider === 'ai_estimate' ? <Text style={s.small}>{food.macroSource === 'web' ? 'Macros issues de la fiche du produit' : 'Macros moyennes estimées'}</Text> : null}{food.sourceUrl ? <Button text="Voir la source des macros" variant="outline" onPress={() => { void Linking.openURL(food.sourceUrl!); }} /> : null}</Card>
