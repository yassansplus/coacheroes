...
export function NutritionHistory({ meals, onSelect, notify }: { meals: Meal[]; onSelect: (meal: Meal) => void; notify: (message: string) => void }) {
  ...
  return <>
    ...
    <CalorieCard value={value} target={goals.calories * dayCount} label={period === 'day' ? dateFromKey(date).toLocaleDateString('fr-FR', { weekday: 'long', day: 'numeric', month: 'long' }) : period === 'week' ? 'Bilan de la semaine' : 'Bilan du mois'}><MacroSummary value={value} /></CalorieCard>
